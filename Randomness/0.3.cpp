...
void Walker::step()
{
	...
	float x = xe::randomf(0.00f, 1.00f);

	if (x < 0.4f) coord.x++; //40 % chance of moving to right
	else if (x<0.6f) coord.x--;
	else if (x<0.8f) coord.y++;
	else coord.y--;
...
public:
	Canvas()
	{
		InitWindow(1200, 900, "Random Walker (Moves Right)");
	}
